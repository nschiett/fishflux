...
## Note that in this example, there is a large difference between FishBase estimates and otolith analysis. This illustrates how important it is to check the parameter estimates you're using.
```
\newline
...
# example
metabolism <- fishflux::metabolism(family= "Pomacentridae", temp = 27, troph_m = 2)
```
```{r}
print(metabolism)
```
...
## Plot one variable:
fishflux::plot_cnp(model, x = "tl", y = "Fp", probs = c(0.5, 0.8, 0.95))
...
### Plot results 
The function `limitation()` returns the proportion of iterations of the model simulation that had limitation of C, N and P respectively.
\newline
```{r, message=FALSE, warning=FALSE}
## Plot one variable:
