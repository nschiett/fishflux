...
# forcing number of cores to be 1 so it works for all OS on CI
old <- options()
options("mc.cores" = 1)
```
## Introduction
...
```{r,message=TRUE,echo=TRUE}
# example
find_lw("Zebrasoma scopas", mirror = "se")
```
\newline
...
growth_params("Sargocentron microstoma", otolith = FALSE)
```
...
limitation(model)
## Plot one variable:
plot_cnp(model,  y = "Fp", x = "tl", probs = c(0.5, 0.8, 0.95))
## Plot multiple variables:
plot_cnp(model,  y = c("Fp", "Gp", "Ip", "Wp"), x = "tl", probs = 0.5)
```
